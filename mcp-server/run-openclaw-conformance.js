...
const REQUIRED_AUDIENCE = (process.env.OPENCLAW_REQUIRED_AUDIENCE || 'openclaw').trim().toLowerCase();
...
async function issueToken(overrides = {}) {
  const payload = {
    agent_name: `openclaw-conformance-${Date.now()}`,
    ttl_hours: 1,
    scope: ['read'],
...
async function revokeToken(tokenId) {
  await call(`/api/agent/credentials/${encodeURIComponent(tokenId)}/revoke`, {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify({ reason: 'OpenClaw conformance cleanup' })
  });
}
