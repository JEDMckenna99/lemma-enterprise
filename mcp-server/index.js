...
const REQUIRED_AUDIENCE = (process.env.LEMMA_FIREWALL_REQUIRED_AUDIENCE || '').trim().toLowerCase();
const RUNTIME_ID = (process.env.LEMMA_FIREWALL_RUNTIME_ID || 'lemma-firewall-default').trim();
const RUNTIME_ORG_ID = (process.env.LEMMA_FIREWALL_RUNTIME_ORG_ID || 'org_default').trim();
const RUNTIME_ENVIRONMENT = (process.env.LEMMA_FIREWALL_RUNTIME_ENVIRONMENT || 'prod').trim();
const ENABLE_LOCAL_OPS_GATE = String(process.env.LEMMA_ENABLE_LOCAL_OPS_GATE || '0').trim().toLowerCase() === '1';
const LEMMA_CREDENTIAL_ENV = process.env.LEMMA_CREDENTIAL || '';
const LEMMA_CREDENTIAL_FILE = process.env.LEMMA_CREDENTIAL_FILE || '';
const LEMMA_CREDENTIAL = process.env.LEMMA_CREDENTIAL || '';
const FIREWALL_URL = (process.env.LEMMA_FIREWALL_URL || '').replace(/\/+$/, '');
...
async function callApi(endpoint, options = {}) {
  const url = FIREWALL_URL
    ? `${FIREWALL_URL}/firewall/lemma${endpoint}`
    : `${LEMMA_BASE_URL}${endpoint}`;
  const headers = {
    'Content-Type': 'application/json',
    ...(LEMMA_CREDENTIAL ? { 'X-Lemma-Credential': LEMMA_CREDENTIAL } : {}),
    ...(AGENT_TOKEN && !LEMMA_CREDENTIAL ? { 'X-Agent-Token': AGENT_TOKEN } : {}),
    ...options.headers
  };
...
function operationDescriptorForTool(name, args = {}) {
  if (name === 'lemma_api_call') {
    const method = String(args.method || 'GET').toUpperCase();
    const endpoint = normalizePath(args.endpoint) || String(args.endpoint || '');
    const isWrite = ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method);
    const adminCall = endpoint.toLowerCase().startsWith('/api/admin');
    return {
      action: adminCall ? 'api.internal.admin' : (isWrite ? 'api.call.write' : 'api.call.read'),
      risk: adminCall ? 'critical' : (isWrite ? 'high' : 'low'),
      resource: endpoint || '/',
    };
  }
  if ([
    'lemma_click', 'lemma_type', 'lemma_select', 'lemma_checkbox',
    'lemma_fill_form', 'lemma_wait_for', 'lemma_test_site_registration',
    'lemma_test_api_key_generation', 'lemma_test_agent_token_flow', 'lemma_run_ui_test'
  ].includes(name)) {
    const currentPath = normalizePath(new URL(page?.url?.() || `${LEMMA_BASE_URL}/`).pathname) || '/';
    return {
  ...
  const readPath = requestedPathForTool(name, args) || '/';
  return {
    action: 'browser.read',
    risk: 'low',
    resource: readPath,
...
async function lemmaCredentialValue() {
  if (LEMMA_CREDENTIAL_ENV && LEMMA_CREDENTIAL_ENV.trim()) {
    return LEMMA_CREDENTIAL_ENV.trim();
  }
  if (LEMMA_CREDENTIAL_FILE && LEMMA_CREDENTIAL_FILE.trim()) {
    try {
      const raw = await fs.readFile(LEMMA_CREDENTIAL_FILE.trim(), 'utf8');
...
async function runtimeAuthorizeDescriptor(descriptor) {
  const credential = await lemmaCredentialValue();
  if (!credential) {
    return { ok: false, error: 'missing_lemma_credential' };
  }
  const response = await fetch(`${LEMMA_BASE_URL}/api/wallet/runtimes/${encodeURIComponent(RUNTIME_ID)}/authorize`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Lemma-Credential': credential,
      'X-Lemma-Org-Id': RUNTIME_ORG_ID,
      'X-Lemma-Environment': RUNTIME_ENVIRONMENT,
    },
    body: JSON.stringify(descriptor),
  });
  const payload = await response.json().catch(() => ({}));
  return {
    ok: response.ok && (payload.allowed === true || payload.success === true),
    status: response.status,
    payload,
  };
}
...
async function authorizeThenExecute(toolName, args, executeFn) {
  ...
  if (LEMMA_CREDENTIAL) {
    ...
    if (result && result.content && !result.isError) {
      result.content.push({
        type: 'text',
        text: JSON.stringify({
          authz: { allow: true, auth_mode: 'credential_local_verify' }
        }, null, 2)
      });
    }
    return result;
  ...
  if (ENABLE_LOCAL_OPS_GATE) {
    const descriptor = operationDescriptorForTool(toolName, args || {});
    const runtimeDecision = await runtimeAuthorizeDescriptor(descriptor);
    if (!runtimeDecision.ok) {
      return deny(
        runtimeDecision?.payload?.error || runtimeDecision?.error || 'runtime_authorize_denied',
        {
          runtime_id: RUNTIME_ID,
          status: runtimeDecision?.status || null,
          descriptor,
        },
      );
    }
...
async function main() {
  ...
  console.error(`Credential: ${LEMMA_CREDENTIAL ? 'configured' : 'NOT SET'}`);
  console.error(`Firewall URL: ${FIREWALL_URL || 'NOT SET'}`);
