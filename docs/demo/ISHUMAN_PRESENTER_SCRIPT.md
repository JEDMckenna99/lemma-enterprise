...
Hub narrative: **Create · Verify · Enforce**.
...
3. Verify once and show two distinct site-private PPIDs for ticketing and trials.
4. Say: same lemma.id, unlinkable IDs; signed presentations verified offline on each site backend.
...
6. **Set assurance**: require human proof on ticketing; show valid-but-insufficient passkey proof; complete IDV; re-verify with **same PPID** at `ishuman`.
