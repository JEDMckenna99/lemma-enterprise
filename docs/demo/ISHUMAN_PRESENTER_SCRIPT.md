...
6. Show local backend presentation verification and the privacy-safe billing
   event fields.
7. On the presale code demo (https://tickets-demo.lemma.id/?tour=presale):
   ...
   - **Optional** — Simulate site risk flag → fan completes fresh IDV (`verifyFreshForBackend`) as penalty → retry at `ishuman`.
