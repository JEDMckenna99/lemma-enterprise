...
## 🎯 **Protocol Overview (Research + Implementation Notes)**

The Lemma protocol describes privacy-preserving verification using **OPRF-Cascaded Bloom Filters** with **Decentralized Identity (DID)** credentials. This document mixes implemented behavior and forward-looking optimization research.

> Read this as a technical design reference, not a product readiness guarantee. For launch status, see `docs/status/GA_GATE_STATUS.md`.

### **Performance Notes**
- Local verification can be very low-latency in optimized paths.
- Some values in this document represent lab benchmarks or projected hardware-assisted paths.
- Throughput and latency depend on runtime, device, browser, network, and deployment configuration.
- Compatibility and readiness should be validated against current production evidence and GA gates.
...
### **Current Status**
- Performance varies by environment and optimization path.
- Hardware-assisted values are not baseline expectations for all deployments.
- Treat this section as engineering benchmarks, not SLA commitments.
...
**After Updates:**
- Low-latency verification has been demonstrated in optimized paths
- Core components compile and benchmark in tested environments
...
- **⚠️ Launch Readiness**: Refer to GA gate artifacts for deployment decisions
...
### **Microsecond Verification Objective**
Microsecond-level verification has been demonstrated in specific optimized paths:
...
- **Production Deployment**: Subject to GA gate closure and operational validation
