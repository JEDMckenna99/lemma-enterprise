...
- Audience: Security reviewers, wallet engineers, verifier maintainers,
...
passkey-secured wallet. Daily authentication and human assurance are separate
...
| Passkey-tier credential | Continuity with a wallet-bound provisional or known person root at `assurance=passkey` | IDV-backed humanity or one-person-per-account assurance | Verify signed presentation, site binding, expiry, revocation, and assurance |
...
| Wallet Ed25519 assertion | Possession of an already authorized wallet device signing key and binding of the declared fields to a server challenge | Passkey user verification, human assurance, site administration, or authority to enroll itself | Verify enrolled non-revoked device key, nonce, wallet binding, purpose-bound fields, expiry, and replay state |
| Wallet session cookie or unlock token | Server-issued cached unlock state for the wallet named in the signed token | Fresh passkey proof, unique humanity, permission, site ownership, or authority to create new wallet keys | Verify server signature, expiry, intended audience and purpose; restrict to explicitly permitted low-risk operations |
...
| Recovery proof | Completion of the specifically documented recovery ceremony and authority to bind a replacement authenticator to the canonical person | Authority derived from email, wallet ID, or IDV session ID alone | Verify one-time recovery state, fresh IDV when required, replacement passkey possession, exact account/site binding, expiry, and atomic consumption |
...
| Site API key | Possession of a relying-site backend credential scoped to one registered site | End-user identity, wallet control, or authority over another site | Compare using the authoritative hash-only key store, enforce active status and exact site ownership |
...
5. A wallet identifier is never accepted as proof of wallet control.
...
7. A new device key cannot authorize its own enrollment into an existing
   wallet.
8. Cookie-backed wallet state cannot authorize a mutation unless that
   operation is explicitly listed for wallet-session principals.
...
- **Wallet unlock state:** local encrypted state or a server-signed convenience
...
- **IDV session:** provider and server state for one verification attempt. Its
  identifier is not a recovery or wallet credential.

## Authority-creation rule

Every operation that creates or changes wallet, identity, tenant, billing, or
...
- Treating email, phone number, device fingerprint, wallet ID, or PPID as
