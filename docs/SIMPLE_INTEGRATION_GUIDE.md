...
- **Client-side security**: Wallet secret never touches the server
...
if (redirectResult?.success) {
    // User just authenticated - get their PPID
...
app.post('/api/auth/lemma-verify', async (req, res) => {
    ...
    // PPID format: did:lemma:ppid_<64-char-hex>
    // This is unique per user per site - use it as their identifier
    
    let user = await db.users.findOne({ lemma_ppid: ppid });
    
    if (!user) {
        // New user - create account
        user = await db.users.create({ lemma_ppid: ppid });
    }
...
<body>
    ...
    (async function() {
        ...
        async function handleSignIn(ppid) {
...
5. SDK decrypts locally, derives site-specific PPID
   ↓
6. Your backend receives PPID, creates/finds user
```
...
- **Wallet secret**: Generated on first device, never leaves client
- **PPID derivation**: `HMAC(wallet_secret, site_domain)` - computed locally
- **No tracking**: Each site gets a different PPID
- **No server storage**: We never see your wallet secret or PPIDs
