...
> **Verification is Not Data Sale**: A Technical Guarantee

## Executive Summary

Lemma provides identity verification services without selling or exposing user data. This document explains the architectural guarantees that make this distinction enforceable, not just a policy promise.
...
### 2. Cross-Site Correlation is Cryptographically Impossible
...
- Lemma cannot correlate users across sites (PPID derived client-side)
...
| Site visit history | **No** | Impossible to collect (client-side verification) |
| Verification results | **No** | Impossible to collect |
...
| Cross-site correlation? | Usually possible | Cryptographically impossible |
...
Lemma is architected so that selling user data is **technically impossible**, not just against policy:

1. **Verification is local** → Lemma can't observe it
2. **PPIDs are site-specific** → Cross-site correlation impossible  
