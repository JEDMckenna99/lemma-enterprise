...
We introduce *Trust Calculus*, a formal system for constructing, composing, and revoking cryptographic trust assertions between arbitrary communicating nodes. A Trust Calculus *lemma* is an Ed25519-signed claim that any node can verify locally in under a millisecond, without contacting the issuing authority. Lemmas compose: if node A trusts B and B trusts C, the chain A→B→C is itself a locally-verifiable proof, with the effective authority of the composed chain equal to the intersection of all scopes along the path. Revocation of any lemma in a chain cryptographically invalidates all downstream compositions, propagated through cascaded Bloom filters that preserve the privacy of both the revoking party and the checking party. Pairwise Pseudonymous Identifiers (PPIDs) derived via HMAC-SHA256 ensure that the same node participating in multiple trust graphs cannot be correlated across contexts.

We prove three safety properties: (1) *monotonic attenuation*—composition can only narrow authority, never widen it; (2) *revocation completeness*—revoking any link invalidates all compositions containing it; and (3) *privacy under composition*—no coalition of verifiers can link a node's participation across independent trust graphs. We benchmark the reference implementation against OAuth 2.0 token introspection, mTLS certificate verification, and blockchain-based credential systems, demonstrating 1–3 orders of magnitude improvement in verification latency and zero network dependency at the point of trust evaluation.
...
The entire system evaluates locally. A verifying node needs only the issuer's public key (32 bytes) and a periodically-synced Bloom filter to make trust decisions with cryptographic certainty, in constant time, with no network calls.
...
**Complexity**: $O(k)$ Ed25519 verifications + $O(k)$ Bloom filter lookups, where $k$ is chain depth. Each Ed25519 verification is $< 500\mu s$ (WebCrypto); each Bloom filter lookup is $< 5\mu s$. For a chain of depth 10: $< 5ms$ total. **No network calls.**
...
#### Property 3: Unlinkability Under Composition
...
Revocation status is encoded in a cascaded Bloom filter with three levels, balancing capacity, accuracy, and distribution size:
...
Revocation events propagate through three mechanisms:
...
3. **Periodic (client-side)**: Bloom filter re-download via `GET /api/revocation/bloom-filter`, configurable interval (default: 60 seconds).

The maximum revocation propagation delay is bounded by the client sync interval.
...
where $s_n$ is the node's 256-bit wallet secret (generated randomly at wallet creation, stored encrypted in IndexedDB) and $d = \text{canonicalize}(\text{hostname})$ extracts the lowercase hostname from the domain. The derivation is performed entirely client-side using the WebCrypto API; the wallet secret never leaves the client.
...
where:
- $C_{\text{Ed25519}} \leq 500\mu s$ (WebCrypto, primary), $\leq 100\mu s$ (WASM, optional), $\leq 2ms$ (pure JS fallback)
- $C_{\text{bloom}} \leq 5\mu s$ (SHA-256 double hash + bit lookups)
- $C_{\text{expiry}} < 1\mu s$ (integer comparison)

For the WebCrypto implementation (production default):
...
With caching of previously-verified issuer public keys:
...
Trust Calculus is comparable to JWT's verification speed while adding composition, revocation, and privacy—properties that JWT lacks. It matches UCAN's composability while adding revocation and privacy—the critical gaps in UCAN. It exceeds all other systems in the combination of all five properties.
...
No existing system answers all five questions. OAuth tokens are not composable. API keys have no scope attenuation. Blockchain credentials are too slow for real-time agent interaction.
...
2. **Verification Library** (any-side): Pure cryptographic verification of lemmas and chains. Implemented in three forms:
   - JavaScript (browser, primary): WebCrypto API for Ed25519 and HMAC-SHA256, with `@noble/ed25519` fallback
...
Trust Calculus provides a formal system for composable trust between arbitrary communicating nodes. Its core insight is that cryptographic signatures are inherently composable: if each link in a chain is locally verifiable, the chain itself is locally verifiable, and this property is recursive. By combining Ed25519 signatures (for unforgeable assertions), scope intersection (for monotonic attenuation), cascaded Bloom filters (for privacy-preserving revocation), and HMAC-SHA256 PPIDs (for cross-context unlinkability), Trust Calculus achieves a combination of properties—local verification, composition, attenuation, revocation, and privacy—that no existing system provides.

The practical consequence is a shift in the economics of trust. When verification costs under a millisecond and zero network calls, trust ceases to be a gate that nodes pass through once and instead becomes an ambient property of every interaction. Every message carries its own proof. Every delegation carries its own constraints. Every chain traces back to a hardware-attested human decision.

As computing shifts toward autonomous agents, federated services, and ubiquitous devices, the need for composable machine-to-machine trust will grow by orders of magnitude. Trust Calculus offers a foundation: simple enough to verify in microseconds, formal enough to prove safety properties, and general enough to apply wherever nodes must communicate trusted facts.
