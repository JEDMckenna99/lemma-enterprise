...
By default, sites receive different identifiers, which helps reduce cross-site linkability. Correlation risk still depends on what additional identifiers each site collects or shares.
```
...
Lemma:
  Runtime verification can happen from user-held credentials
  Credentials are stored in USER'S wallet
  Server-side state may still exist for issuance, revocation, and optional session flows
```
...
- ✅ **Session hijacking risk reduced** - credential-first flows can minimize central session state; optional server sessions still require standard session hardening
...
```javascript
// Client-side verification path (server sessions optional by integration)
...
- [Quick Start](../integration/QUICK_START_SIMPLE_LOGIN.md) - 5-minute integration
- [Architecture](../architecture/ARCHITECTURE_WALLET_FIRST.md) - Wallet-first design
- [Whitepaper](../architecture/WHITEPAPER_DIGITAL_LEMMAS.md) - Technical foundation
