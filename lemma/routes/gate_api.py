...
def gate_status():
    """
    Check the current gate status for the user with configurable security levels
    ...
    try:
        ...
        if user_session_data['verified_user'] and user_session_data['verified_human']:
            ...
            if verification_age > config['max_verification_age']:
                ...
                return jsonify({
                    'success': True,
                    'gate_action': 'require_reverification',
...
def verify_credentials():
    ...
    try:
        ...
        if not revocation_status['valid']:
            ...
            return jsonify({
                'success': False,
                'gate_action': 'credential_revoked',
        ...
        if config['require_fresh_verification'] or force_reverification:
            ...
            if existing_verification_time:
                ...
                if verification_age < 300:  # Less than 5 minutes old
                    ...
                    return jsonify({
                        'success': False,
                        'gate_action': 'require_fresh_verification',
...
@gate_api.route('/api/gate/revoke-credential', methods=['POST'])
@csrf_protect()
@require_api_key
@rate_limit
...
@gate_api.route('/api/gate/force-reverification', methods=['POST'])
@csrf_protect()
@require_api_key
@rate_limit
...
@gate_api.route('/api/gate/security-levels', methods=['GET'])
@rate_limit
    ...
    try:
        ...
        config = {
            'endpoints': {
                ...
                'revoke_credential': '/api/gate/revoke-credential',
                'force_reverification': '/api/gate/force-reverification',
                'security_levels': '/api/gate/security-levels'
            },
