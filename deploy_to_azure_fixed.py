#!/usr/bin/env python3
"""
Azure Deployment Script for Lemma Enterprise (Fixed Version)

This script automates the deployment of the Lemma Human Verification System to Azure App Service
with proper configuration to resolve module not found errors.
"""
import os
import sys
import subprocess
...
def check_prerequisites():
    ...
    print("✅ Azure CLI is installed (detected from previous command)")
...
def deploy_to_azure(zip_file, resource_group, app_name, plan_name, location, env_vars):
    """Deploy the package to Azure App Service with proper configuration."""
    ...
    # Create web app with Python 3.9 runtime
    ...
    print("📝 Configuring Python settings")
    subprocess.run([
        'az', 'webapp', 'config', 'set',
        '--resource-group', resource_group,
        '--name', app_name,
...
def main():
    ...
    print("=== Lemma Enterprise Azure Deployment (Fixed Version) ===")
