...
class QRHealthMonitor:
        ...
        try:
            if psutil is None:
                return HealthMetric(
                    ...
                    value=None,
                    unit="%",
                    message="psutil not available in runtime"
                )
    ...
    def _check_api_health(self) -> HealthMetric:
        """Check API health from live telemetry instead of simulated values."""
        try:
            from monitoring.request_telemetry import slo_snapshot_last_5m

            snapshot = slo_snapshot_last_5m()
            response_time_ms = snapshot.get('p95_latency_ms_5m')
            error_rate = snapshot.get('error_rate_5m_percent') or 0.0
            ...
            if response_time_ms is None:
                return HealthMetric(
                    ...
                    value=None,
                    unit="ms",
                    message="Insufficient request telemetry for API latency"
                )

            if response_time_ms >= self.alert_thresholds['api_response_time']['critical'] or error_rate >= 5.0:
                status = "critical"
                message = f"API p95 latency {response_time_ms:.1f}ms, 5xx rate {error_rate:.2f}%"
            elif response_time_ms >= self.alert_thresholds['api_response_time']['warning'] or error_rate >= 2.0:
                status = "warning"
                message = f"API p95 latency {response_time_ms:.1f}ms, 5xx rate {error_rate:.2f}%"
            else:
                status = "healthy"
                message = f"API p95 latency healthy at {response_time_ms:.1f}ms"
    ...
    def _check_qr_generation_health(self) -> HealthMetric:
        """QR generation benchmark telemetry is not sampled in this process."""
        return HealthMetric(
            ...
            message="No runtime QR generation benchmark signal available"
        )
    
    def _check_qr_verification_health(self) -> HealthMetric:
        """QR verification benchmark telemetry is not sampled in this process."""
        return HealthMetric(
            ...
            message="No runtime QR verification benchmark signal available"
        )
    
    def _check_webassembly_health(self) -> HealthMetric:
        """Check crypto runtime module availability."""
        try:
            ...
            if _issuer:
                status = "healthy"
                message = "Crypto runtime module loaded and ready"
                value = "available"
            else:
                status = "critical"
                message = "Crypto runtime module not available"
                value = "unavailable"
            
            return HealthMetric(
                name="crypto_runtime_status",
                status=status,
                value=value,
                unit="",
                message=message
            ...
            return HealthMetric(
                ...
                value="error",
                unit="",
                message=f"Crypto runtime health check failed: {e}"
            )
