...
def _api_rate_limit_degraded_mode() -> str:
    """
    Behavior when Redis-backed limiter is unavailable.
    - memory (default): local in-process fixed-window fallback
    - fail_open: allow requests without limiting
    """
    mode = (os.getenv("LEMMA_API_RATE_LIMIT_DEGRADED_MODE") or "memory").strip().lower()
    if mode not in {"memory", "fail_open"}:
        return "memory"
    return mode
...
def _memory_window_increment(rate_key: str, period: int) -> tuple[int, int]:
    ...
    now = int(time.time())
    with _memory_lock:
        entry = _memory_counters.get(rate_key)
        if not entry or now >= entry["reset_at"]:
            entry = {"count": 0, "reset_at": now + period}
            _memory_counters[rate_key] = entry
        entry["count"] += 1
...
def rate_limit(
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            degraded_mode = _api_rate_limit_degraded_mode()

            # Skip/replace if Redis not available
            if not REDIS_AVAILABLE:
                if degraded_mode == "fail_open":
                    logger.warning("Rate limiting skipped for %s (Redis unavailable)", f.__name__)
                ...
                current, ttl = _memory_window_increment(rate_key, period)
                if current > limit:
                    return jsonify({
            ...
            try:
                ...
                # Deterministic in-process fallback to avoid auth-route 500s.
                if key_func:
                    key = key_func()
                else:
                    key = request.remote_addr or 'unknown'
...
def check_rate_limit(key: str, max_requests: int, window_seconds: int) -> bool:
    ...
    if not REDIS_AVAILABLE:
        if degraded_mode == "fail_open":
            return True
        current, _ttl = _memory_window_increment(f'rate_check:{key}', window_seconds)
