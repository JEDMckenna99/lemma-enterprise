...
Didit returns the issuing country as an ISO 3166-1 alpha-3 code (e.g. ``ESP``)
in ``decision.id_verifications[].issuing_state``. Lemma's document-root
canonicalization (``api/identity_roots.normalize_country``) requires a
two-letter alpha-2 code (e.g. ``ES``). This module provides a dependency-free
conversion so the didit IDV rail can feed the existing document-root pipeline.

The table is reference data; it is exercised by unit tests that pin a handful of
representative conversions (ESP->ES, USA->US, GBR->GB).
"""
...
def alpha3_to_alpha2(code: str) -> str | None:
    """Return the ISO alpha-2 code for an alpha-3 input, or None if unknown.

    Already-alpha-2 inputs are passed through when they are a known alpha-2
    value, so callers can tolerate providers that send either form.
    """
    raw = (code or "").strip().upper()
    if not raw:
        return None
    if len(raw) == 3:
        return ALPHA3_TO_ALPHA2.get(raw)
    if len(raw) == 2 and raw in set(ALPHA3_TO_ALPHA2.values()):
        return raw
    return None
