...
def _is_non_prod_mode() -> bool:
    env = (os.getenv("FLASK_ENV") or os.getenv("APP_ENV") or os.getenv("ENV") or "").strip().lower()
    return env in {"", "development", "dev", "test", "local"}


def _allow_sdk_demo_features() -> bool:
    raw = (os.getenv("LEMMA_SDK_ALLOW_DEMO") or "").strip().lower()
    return raw in {"1", "true", "yes", "on"}


def _get_network_registry_url() -> str:
    configured = (os.getenv("LEMMA_NETWORK_REGISTRY_URL") or "").strip()
    if configured:
        return configured
    if _is_non_prod_mode():
        return "http://localhost:5000"
    # Production default is closed when not explicitly configured.
    return ""
...
def distribute_did_to_network(did: str, public_key: str, issuer_info: dict) -> bool:
    ...
    """
    try:
        registry_url = _get_network_registry_url()
        if not registry_url:
            logger.warning("Network DID distribution skipped: LEMMA_NETWORK_REGISTRY_URL not configured")
            return False

        response = requests.post(
            f"{registry_url}/api/network/register-did",
            headers={
...
def distribute_revocation_to_network(credential_id: str, bloom_hash: str, reason: str) -> bool:
    ...
    try:
        registry_url = _get_network_registry_url()
        if not registry_url:
            logger.warning("Network revocation distribution skipped: LEMMA_NETWORK_REGISTRY_URL not configured")
            return False

        response = requests.post(
            f"{registry_url}/api/network/register-revocation",
            headers={
                'Authorization': f'Network {_get_network_auth_key()}',
                'Content-Type': 'application/json'
            },
...
def _extract_api_key_from_request() -> str:
    """
    Accept either Authorization: Bearer <key> or X-API-Key for SDK endpoints.
    """
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not api_key:
            return jsonify({
                'success': False,
                'error': 'invalid_auth',
                'message': 'API key required via Authorization: Bearer <key> or X-API-Key'
            }), 401

        # Accept platform internal keys only when explicitly allowed in non-production.
        platform_keys = [
            'lemma_platform_production_key_2024',
            'lemma_platform_internal_key_2024'
        ]

        allow_demo = _allow_sdk_demo_features() and _is_non_prod_mode()
        if allow_demo and (api_key.startswith('demo-') or api_key == 'client-demo-key' or api_key in platform_keys):
        ...
        if heroku_api_key and hmac.compare_digest(api_key, heroku_api_key):
            request.api_key = api_key
...
def create_demo_identity_session(user_id: str, return_url: str, inline_mode: bool) -> dict:
    ...
    logger.warning("⚠️ Using demo identity verification session (non-production)")
...
def sdk_health():
    ...
    return jsonify({
        ...
        'environment_mode': 'non_production' if _is_non_prod_mode() else 'production',
        'demo_mode_enabled': _allow_sdk_demo_features() and _is_non_prod_mode(),
        'network_registry_configured': bool(_get_network_registry_url()),
