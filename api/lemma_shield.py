...
def create_lemma_credential(user_id: str, stripe_session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"📋 Claims: packageType={claims.get('packageType')}, "
                   f"isHuman={claims.get('isHuman')}, "
                   f"verificationMethod={claims.get('verificationMethod')}")
            ...
            try:
                ...
                try:
                    ...
                    record_revocation(
                        ...
                        reason_code="user_revoke_credential",
                        org_id=session.get("org_id", "org_default"),
                        environment=session.get("environment", "prod"),
                    )
                except Exception as store_exc:
                    logger.warning("revocation delta store write failed (bloom still applied): %s", store_exc)
