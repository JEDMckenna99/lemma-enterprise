...
def create_credential_from_stripe_verification_rust(user_id: str, session_id: str) -> Dict[str, Any]:
    """
    Create minimal identity lemma using Rust engine with only 3 essential claims after Stripe Identity verification
    
    This uses the Rust crypto engine to create the minimal lemma that delivers maximum customer value:
    1. packageType: 'identity' - Routes to identity verification package
    2. isHuman: True - The critical bot shield claim
    3. verificationMethod: 'stripe_identity' - Proves high-assurance Stripe KYC completion
    """
    global rust_engine, RUST_ENGINE_AVAILABLE
    ...
    try:
        ...
        logger.info(f"✅ Rust engine created identity lemma {credential['id']} with 3 essential claims")
...
def create_credential_from_stripe_verification(user_id: str, session_id: str) -> Dict[str, Any]:
    """
    Create minimal identity lemma with only 3 essential claims after Stripe Identity verification
    ...
    try:
        ...
        try:
            ...
            if verification_session.get('success'):
                ...
                if status == 'verified':
                    # CREDENTIAL GENERATION: Create minimal identity lemma with 3 essential claims
                    credential = create_credential_from_stripe_verification(user_id, session_id)
                    
                    # RUST ENGINE PROCESSING: Route credential through Rust engine for validation
                    rust_verification_result = None
                    rust_verification_time_us = 0
                    ...
                    if RUST_ENGINE_AVAILABLE and rust_engine:
                        try:
                            ...
                            # Verify the credential through Rust engine
                            rust_verification_result = rust_engine.verify_credential(credential_json)
                            ...
                            logger.error(f"❌ Rust engine verification failed: {e}")
                            rust_verification_result = None
                    ...
                    session['rust_verification_time_us'] = rust_verification_time_us
                    
                    # Store minimal credential in session
                    ...
                    return jsonify({
                        ...
                        'flow_path': 'shield_flow_completed',
                        'engine': 'rust_backend' if RUST_ENGINE_AVAILABLE else 'python_backend',
                        'rust_verification_time_us': rust_verification_time_us,
                        'claims_count': len(credential['claims']),
                        'message': 'Stripe Identity verification completed - minimal identity lemma created'
                    })
