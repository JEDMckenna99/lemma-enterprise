...
def _limiter_degraded_mode() -> str:
    """
    Behavior if limiter backend fails at runtime.
    - fail_open (default): allow request and avoid auth-route 500 spikes
    - strict: raise backend errors
    """
    mode = (os.getenv("LEMMA_AUTH_LIMITER_DEGRADED_MODE") or "fail_open").strip().lower()
    if mode not in {"fail_open", "strict"}:
        return "fail_open"
    return mode
...
def _redis_storage_uri() -> str:
    """
    Resolve limiter storage URI with deterministic fallback behavior.
    """
    if not REDIS_URL:
        logger.warning("Rate limiter using in-memory storage (REDIS_URL missing)")
        return "memory://"
    ...
    try:
        ...
        logger.warning("Rate limiter Redis probe failed, using in-memory storage: %s", e)
        return "memory://"
...
def create_limiter(app):
    ...
    # Prevent transient Redis limiter outages from raising 500s in before_request.
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if _limiter_degraded_mode() == 'fail_open':
                    logger.warning("Rate limiter backend degraded; allowing request: %s", e)
                    return f(*args, **kwargs)
                # strict mode: surface backend error
                raise
